...
define('LEAD_STATUS_REMARK_MAP', [
    "Open"=>['Hot','Warm','Cold'],
    "Qualified"=>[],
    "Disqualified"=>['Not Reachable','No Requirement','Incorrect Contact','Junk'],
    "Lead lost"=>['No Response','Competition','On Hold'],
    "Junk lead"=>[]
    // Add other status options here
]);

/**
 * Filter Lead Remark Options by Lead Status
 * Used in Lead Form
 * @param  array $arrLead
 * @param  int $intLeadStatus
 * @return string return filterd Lead
 */
function lead_filter_remark_options( $arrLead, $intLeadStatus ){
    ...
    foreach ($query->result() as $row) {
        $arrStatuses[$row->id] = $row->name;
    }

    if( isset( $arrStatuses[$intLeadStatus], LEAD_STATUS_REMARK_MAP[$arrStatuses[$intLeadStatus]] ) ){
        $arrLead['options'] = implode(',', LEAD_STATUS_REMARK_MAP[$arrStatuses[$intLeadStatus]] );
    }

    return $arrLead;
}

function lead_get_remark_options(){
    $arrStatusRemarkMap=LEAD_STATUS_REMARK_MAP;
    ...
    $statusToIdMap = array();
    foreach ($query->result() as $row) {
        $statusToIdMap[$row->name] = $row->id;
    }
    
    $returnArray = [];

    // Iterate through LEAD_STATUS_REMARK_MAP
    foreach (LEAD_STATUS_REMARK_MAP as $statusName => $remarks) {
        // Get the corresponding ID for the status name
        $statusId = $statusToIdMap[$statusName];
        
        // Assign the remarks array to the ID in the result array
        $returnArray[$statusId] = $remarks;
    }
