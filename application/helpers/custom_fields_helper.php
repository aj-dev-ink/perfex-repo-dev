 ...
 * @return mixed
 */
function render_custom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [], $whereNotIn = [])
{
...
function render_cust_field( $field, $isWrap=true, $showEditLink = false, $rel_id = false, $items_cf_params = [] ){
    ...
    if (!$items_add_edit_preview && !$items_applied) {
        if( $isWrap ){
            $fields_html .= '<div id="'.$customDivId.'" class="col-md-' . $field['bs_column'] . '">';
        } else{
            $fields_html .= '<div id="'.$customDivId.'">';
        }
    } elseif ($items_add_edit_preview) {
        $fields_html .= '<td class="custom_field" data-id="' . $field['id'] . '">';
    } elseif ($items_applied) {
