...
class Imap_etm
{
     ...
     * @param $withbody without body
     */
    public function getMessages($withbody = true , $last_sequence_id = 0 )
    {
    ...
    public function convertToUtf8($text, $fromCharset)
    {
        ...
        if ( empty( $text ) || empty( $fromCharset ) )
            return $text;

        if ( !empty( $fromCharset ) )
        {

            $fromCharset = str_replace( 'utf-8utf-8' , 'utf-8' , $fromCharset );
            $fromCharset = str_replace( 'utf-8utf-8' , 'utf-8' , $fromCharset );
            $fromCharset = str_replace( 'utf-8utf-8' , 'utf-8' , $fromCharset );
            $fromCharset = str_replace( 'utf-8utf-8' , 'utf-8' , $fromCharset );
            ...
            $fromCharset = str_replace( 'UTF-8' , 'utf-8' , $fromCharset );
        }
        ...
        $invalidCharsets = [
            'default',
            'defaultGB2312default',
            'gb2312',
            'ius-ascii',
            'us-asciius-ascii',
        ];

        if (stripos($fromCharset, 'utf-8') !== false){
            $fromCharset = 'UTF-8';
        }

        if (in_array($fromCharset, $invalidCharsets)) {
            $fromCharset = 'UTF-8';
        }
    ...
    protected function getAttachments($imap, $mailNum, $uID, $part, $partNum)
    {
        ...
        if (isset($part->parts)) {
            ...
            if (strtolower($part->disposition) == 'attachment' || strtolower($part->disposition) == 'inline') {
                ...
                if ( !empty( $part->dparameters ) && !empty( $part->dparameters[0]->value ) )
                {

                    $attachmentDetails = [
                        'name'    => $part->dparameters[0]->value,
                        'partNum' => $partNum,
                        'enc'     => !empty( $partStruct->encoding ) ? $partStruct->encoding : '',
                        'size'    => !empty( $part->bytes ) ? $part->bytes : '',
                        'mailNum' => $mailNum,
                        'uid'     => $uID,
                    ];
