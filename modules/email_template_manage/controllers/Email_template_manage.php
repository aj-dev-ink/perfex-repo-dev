...
class Email_template_manage extends AdminController
{
    ...
    public function template_content()
    {
        ...
        if( !empty( $template_data ) )
        {
            ...
            if ( in_array( $rel_type , [ 'forward' , 'reply' ] ) )
            {

                $inbox_mail = $this->email_template_manage_model->get_email_detail( $rel_id );

                $from_text = "$inbox_mail->from_name <".$inbox_mail->from_email.">";

                $forward_text = _l('email_template_manage_email_reply_content', [
                    $from_text ,
                    _dt( $inbox_mail->mail_date )
                ] );

                $mail_description   = '&nbsp;<br/><br/><br/><br/><br/>'.$forward_text.'<blockquote style="margin:0px 0px 0px 1ex;border-left:1px solid rgb(204,204,204);padding-left:2ex">'.$inbox_mail->message.'</blockquote>';


                $template_data->template_content .= $mail_description;
    ...
    public function imap_settings()
    {
        ...
        $data["title"] = _l('email_template_manage_imap_setting');

        $this->load->view('inbox/v_imap_manage',$data);

    }

    public function imap_setting_lists()
    {

        if( $this->input->is_ajax_request() )
        {

            $sTable = db_prefix().'email_template_imap_settings';
            ...
            foreach ($rResult as $aRow){
                ...
                $numberOutput .= '<a href="#" onclick="fnc_imap_setting_dlg( '. $aRow['id'] .'  ); return false;" >' . _l('edit') . '</a>';

                $numberOutput .= ' | <a class="text-danger _delete" href="'.admin_url('email_template_manage/imap_delete/'.$aRow['id']).'" > '._l('delete').' </a>';
                ...
                $toggleActive = '<div class="onoffswitch" data-toggle="tooltip" data-title="Status">
                            ...
                            <input type="checkbox" data-switch-url="' . admin_url() . 'email_template_manage/imap_change_status" name="onoffswitch" 
                                    class="onoffswitch-checkbox" id="snack_' . $aRow['id'] . '" 
                                    data-id="' . $aRow['id'] . '" ' . ($aRow['status'] == 1 ? 'checked' : '') . '>
    ...
    public function imap_detail( $detail_id = 0 )
    {
        ...
        if( $this->input->is_ajax_request() )
        {
            ...
            $data['data'] = $this->email_template_manage_model->get_imap_detail($detail_id);
            ...
            if ( empty( $data['data'] ) )
            {
                ...
                $data['data']->company_name = '';
                $data['data']->imap_port = '';
                $data['data']->imap_server = '';
                $data['data']->encryption = 'ssl';
                $data['data']->from_date = '';
                $data['data']->user_name = '';
                $data['data']->password = '';
                $data['data']->is_public = 1;
            ...
            $this->load->view('inbox/v_imap_detail',$data );

        }

    }

    public function imap_save()
    {
        ...
        if ( $this->input->post() )
        {
            ...
            if ( !empty( $id ) )
            {
                ...
                $this->db->where('id',$id)->update( db_prefix().'email_template_imap_settings' , $data );

                $message = _l('updated_successfully', _l('email_template_manage_imap'));

            }
            else
            {

                $this->db->insert( db_prefix().'email_template_imap_settings' , $data );

                $message = _l('added_successfully', _l('email_template_manage_imap'));
        ...
        redirect( admin_url('email_template_manage/imap_settings') );

    }

    public function imap_delete( $id )
    {

        $this->db->where('id',$id)->delete(db_prefix().'email_template_imap_settings');

        set_alert('success' , _l('deleted', _l('email_template_manage_imap')) );

        redirect( admin_url('email_template_manage/imap_settings') );

    }

    public function imap_change_status($id, $status)
    {
        ...
        if ($this->input->is_ajax_request())
        {
            ...
            $this->db->update(db_prefix().'email_template_imap_settings' , [
    ...
    public function inbox()
    {
        ...
        $data["title"] = _l('email_template_manage_inbox');

        $data['imap_settings'] = $this->email_template_manage_model->get_imap_settings();

        $this->load->view('inbox/v_mail_manage',$data);
    ...
    public function inbox_lists(){
        ...
        if( $this->input->is_ajax_request() )
        {
            ...
            $sTable = db_prefix().'email_template_manage_inbox';
            ...
            $from_date  = to_sql_date( $this->input->post('from_date') );
            $to_date    = to_sql_date( $this->input->post('to_date') );
            $imap_id    = $this->input->post('imap_id');
            $inbox_type = $this->input->post('inbox_type');

            if( !empty( $from_date ) )
                $where[] = "AND DATE( mail_date ) >= '$from_date'";

            if( !empty( $to_date ) )
                $where[] = "AND DATE( mail_date ) <= '$to_date'";

            if( !empty( $imap_id ) )
                $where[] = "AND imap_id = $imap_id";


            if( !empty( $inbox_type ) )
            {

                switch ( $inbox_type ) // unred star trash
                {

                    case 'unred' : $where[] = "AND unread = 1"; break ;

                    case 'star' : $where[] = "AND is_stard = 1"; break ;

                    case 'trash' : $where[] = "AND is_trush = 1"; break ;
            ...
            $imap_settings = $this->email_template_manage_model->get_imap_settings();
            ...
            if ( !empty( $imap_settings ) )
            {
                ...
                foreach ( $imap_settings as $imap_setting )
                {

                    $imap_ids .= " , ".$imap_setting->id;

                }

                $where[] = "AND imap_id IN ( $imap_ids )";

            }
            else
                $where[] = "AND imap_id = 0";

            if ( empty( $inbox_type ) || $inbox_type != 'trash' )
                $where[] = "AND is_trush = 0";



            $join = ['LEFT JOIN ' . db_prefix() . 'email_template_imap_settings ON ' . db_prefix() . 'email_template_imap_settings.id = ' . $sTable . '.imap_id'];


            $sIndexColumn = 'id';

            $result = data_tables_init($select, $sIndexColumn, $sTable, $join, $where , [ 'unread' , 'is_stard' , 'is_trush' , 'is_attachment' , 'is_trush' ]);
            ...
            foreach ($rResult as $aRow){
                ...
                $mail_id = $aRow['mailid'];
                $detail_url = admin_url('email_template_manage/inbox_detail/'.$mail_id);

                $opt_data = '<div class="checkbox"><input type="checkbox" class="etm_inbox_checkbox" id="etm_inbox_checkbox_'.$mail_id.'" value="' . $mail_id . '"><label for="etm_inbox_checkbox_'.$mail_id.'"></label></div>';

                if( $aRow['is_stard'] == 0 )
                    $opt_data .= '<a class="btn text-default" data-toggle="tooltip" title="" data-original-title="'._l('email_template_manage_add_star').'" onclick="email_template_manage_update_star( '.$mail_id.' , 1 );"><i class="fa fa-star "></i></a>';
                else
                    $opt_data .= '<a class="btn text-success" data-toggle="tooltip" title="" data-original-title="'._l('email_template_manage_remove_star').'" onclick="email_template_manage_update_star( '.$mail_id.' , 0 );"><i class="fa fa-star"></i></a>';

                if( $aRow['is_trush'] == 0 )
                    $opt_data .= ' <a class="btn text-danger _delete" data-toggle="tooltip" title="" data-original-title="'._l('delete').'" onclick="email_template_manage_update_delete( '.$mail_id.' );"><i class="fa fa-trash"></i></a>';


                if ( $aRow['is_attachment'] == 1 )
                    $opt_data .= ' <span class="fa fa-paperclip"> </span> ';


                $reply_info     = $this->db->select('rel_type')->from(db_prefix().'email_template_manage_mail_logs')->where('send_rel_id',$mail_id)->where('send_rel_type','reply')->get()->row();
                $forward_info   = $this->db->select('rel_type')->from(db_prefix().'email_template_manage_mail_logs')->where('send_rel_id',$mail_id)->where_in('send_rel_type','forward')->get()->row();

                if( !empty( $reply_info ) )
                    $opt_data .= ' <span data-toggle="tooltip" title="" data-original-title="'._l('email_template_manage_replied').'" class="fa fa-reply"> </span> ';

                if( !empty( $forward_info ) )
                    $opt_data .= ' <span data-toggle="tooltip" title="" data-original-title="'._l('email_template_manage_forwarded').'" class="fa fa-share"> </span> ';
                ...
                $row[]  = '<a style="color:#333333 !important" href="'.$detail_url.'">'.$aRow['company_name'].'</a>';

                $row[]  = '<a style="color:#333333 !important" href="'.$detail_url.'">'.$aRow['from_name'].'</a>';

                $row[]  = '<a style="color:#333333 !important" href="'.$detail_url.'">'.$aRow['subject'].'</a>';

                $row[]  = _dt($aRow['mail_date']);


                if( $aRow['unread'] == 1 )
                    $row['DT_RowClass'] = 'tmp_unread';
    ...
    public function inbox_delete()
    {
        ...
        if ( $this->input->is_ajax_request() )
        {
            ...
            $this->db->where('id',$email_id)->update(db_prefix().'email_template_manage_inbox' , ['is_trush' => 1]);
            ...
            else
            {
                ...
                set_alert('success' , _l('deleted', _l('email_template_manage_inbox') ) );

                redirect( admin_url('email_template_manage/inbox') );
    ...
    public function inbox_delete_mail( $email_id = 0 )
    {
    ...
    public function inbox_star()
    {
        ...
        if ( $this->input->is_ajax_request() )
        {
            ...
            $email_id       = $this->input->post('email_id');
            $star_status    = $this->input->post('star_status');

            $this->db->where('id',$email_id)->update(db_prefix().'email_template_manage_inbox' , ['is_stard' => $star_status]);
    ...
    public function inbox_detail( $mail_id = 0 )
    {

        $data['detail'] = $this->email_template_manage_model->get_email_detail( $mail_id );
        if( !empty( $data['detail'] ) )
        {

            $data["title"] = $data['detail']->subject;

            $data['attachments'] = $this->email_template_manage_model->get_email_attachments( $mail_id );


            $data['email_response'] = $this->email_template_manage_model->get_email_response( $mail_id );

            $this->load->view('inbox/v_mail_detail',$data);
    ...
    public function inbox_read_status( $email_id = 0 )
    {

        if ( !empty( $email_id ) )
        {

            $this->db->where('id',$email_id)->update(db_prefix().'email_template_manage_inbox' , ['unread' => 1 ]);


            set_alert('success' , _l('updated_successfully', _l('email_template_manage_inbox') ) );
    ...
    public function inbox_mass()
    {
        ...
        if ( $this->input->is_ajax_request() )
        {
            ...
            $inbox_mails = $this->input->post('inbox_mails');


            if ( !empty( $inbox_mails ) && in_array( $action , [ 'add_star' , 'remove_star' , 'unread' , 'read' , 'delete' ] ) )
            {
                ...
                switch ( $action )
                {
                    ...
                    case 'add_star' :       $update_data['is_stard'] = 1; break;
                    case 'remove_star' :    $update_data['is_stard'] = 0; break;
                    case 'unread' :         $update_data['unread'] = 1; break;
                    case 'read' :           $update_data['unread'] = 0; break;
                    case 'delete' :         $update_data['is_trush'] = 1; break;
                ...
                if( !empty( $update_data ) )
                {
                    ...
                    $this->db->where_in('id',$inbox_mails)->update(db_prefix().'email_template_manage_inbox' , $update_data );
            ...
            echo json_encode( [ 'message' => _l('email_template_manage_success') ] );

        }

    }

    public function response_mail_modal( $mail_id = 0 , $response_type = '' )
    {
        ...
        if ($this->input->is_ajax_request())
        {
            ...
            $detail = $this->email_template_manage_model->get_email_detail( $mail_id );

            $data['mail_subject']   = $detail->subject;

            $data['mail_to']        = $response_type == 'forward' ? '' : $detail->from_email;

            $data['mail_cc']        = $detail->cc;

            $from_text = "$detail->from_name <".$detail->from_email.">";

            $forward_text = _l('email_template_manage_email_reply_content', [
                $from_text ,
                _dt( $detail->mail_date )
            ] );

            $data['mail_description']   = '&nbsp;<br/><br/><br/><br/><br/>'.$forward_text.'<blockquote style="margin:0px 0px 0px 1ex;border-left:1px solid rgb(204,204,204);padding-left:2ex">'.$detail->message.'</blockquote>';

            $data["rel_type"]       = $response_type;
